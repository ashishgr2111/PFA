...
void Des::xor_key(int round, int mode)
{
	round--;
	if(mode == DEC) round = 15 - round;
	for(int i=0;i < 48;i++)
...
void Des::round_function(int round,int mode, bool faulty=false){
	Expansion();
	xor_key(round, mode);
	if(mode == ENC){
...
Mode is the ENC or DEC mode
Output is a unsigned unsigned character stream after running des algorithm
*/
char * Des::run_des(const char *Text,int mode, bool faulty = 0)
{
	int len = strlen(Text);
	...
	for(int nB=0;nB < size / 64; nB++){  /// Iterating over each block of 64 bits
		...
		// concatting left and right parts in temp.
		memcpy(temp,right,sizeof(temp));
		memcpy(temp+32,left,sizeof(left));
		
		inverseIP(); // applying inverse permutation on temp

		// The below block converts bit stream back to character stream.
		// So for 64 bit encryption the output is an 8 length character array.
		int k = 128,d = 0;
		for (int i = 0; i < 8; i++){
			for (int j = 0; j < 8; j++){
				 d = d + inv_ip[i*8+j] * k;
